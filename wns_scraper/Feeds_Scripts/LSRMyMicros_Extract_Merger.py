...
# RunningFlag=ApplicationPath+'Merger_Runningfal.txt'
...
headerStr="Location,GrossSalesAfterDisc,Company,Extraction_date"
...
with open(HTMLParseFile,'r') as f:
    ...
    for line in f:
        ...
        worksheet_SKS.write(row, col+1, float(line.split("||")[1].strip().replace(',','').strip(")").replace("(","-")))
        worksheet_SKS.write(row, col+2, 'SKS')
        worksheet_SKS.write(row, col+3, str(LogicalDate))
        row+=1
...
with open(HTMLParseFile1,'r') as f:
    ...
    for line in f:
        ...
        worksheet_EUG.write(row, col+1, float(line.split("||")[1].strip().replace(',','').strip(")").replace("(","-")))
        worksheet_EUG.write(row, col+2, 'EUG')
        worksheet_EUG.write(row, col+3, str(LogicalDate))
        row+=1
