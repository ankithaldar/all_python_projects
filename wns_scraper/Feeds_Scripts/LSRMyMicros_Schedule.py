...
LogFile=LogPath+'LSRMyMicros_Schedule'+str(Current_DTTM)+'.txt'
...
def main():
    LogFeed.write("LSRMyMicros_Schedule Process has been started at {} \n".format(datetime.datetime.now().replace(microsecond=0)))
    ...
    if (os.path.exists(RunningFlag)):
        ...
        Subject="MyMicros process LSRMyMicros_Schedule.py failed"
    ...
    if (LogicalDate - datetime.timedelta(days=1) >= Current_Date - datetime.timedelta(days=DataLagDays)):
        ...
        if (os.path.exists(RunningFlag)):
            ...
            Subject = "MyMicros process LSRMyMicros_Schedule.py "
            Ob_CommonFunction.SendMail(Alert_TO_DL, msgString, Subject)
        LogFeed.close()
        exit(0)
    else:
        ...
        try:
            ...
            from Feeds_Scripts import LSRMyMicros_Extraction_EUG
            print("call Extraction process SKS")
            from Feeds_Scripts import LSRMyMicros_Extraction_SKS


            print("call Merger")
            from Feeds_Scripts import LSRMyMicros_Extract_Merger
            print("call Insert")
            from Feeds_Scripts import LSRMyMicros_Insert
    ...
    print("LSRMyMicros Process Took '{}' time to complete.\n******BYE******".format(datetime.datetime.now().replace(microsecond=0)-StartTime))
    LogFeed.write("LSRMyMicros Process Took '{}' time to complete.\n******BYE******".format(datetime.datetime.now().replace(microsecond=0)-StartTime))
    LogFeed.close()

main()
'''

  create table DEPT_INTL.T_LSRMYMICROSDAILYFEED
  (LOCATION VARCHAR2(50)  NOT NULL ,
  GrossSalesAfterDisc  VARCHAR2(15)  NOT NULL ,
  COMPANY VARCHAR2(5)NOT NULL  ,  
  EXTRACTION_DATE VARCHAR2(10) NOT NULL )
  
create table DEPT_INTL.T_LSRMYMICROSDAILYFEED_HIST
  (LOCATION VARCHAR2(50)  NOT NULL ,
  GrossSalesAfterDisc FLOAT(15)  NOT NULL ,
  COMPANY VARCHAR2(5)  NOT NULL , 
